...
## 2026-09-06 — Habilitação sai do Perfil e vira parte do Serviço; Serviço ganha editar

**Contexto:** usuário observou a tela de Perfil antes de pedir qualquer mudança ("vou anotar algumas observações antes de você fazer algo"), depois discutiu em várias rodadas: tirar "Habilitações" do Perfil, mover pro formulário de Serviço (só pede o documento quando a categoria exige), sem aprovador bloqueando publicar — só uma notificação pro Admin/Supervisor revisarem quando quiserem, com o documento já visível pro Tutor consultar por conta própria — e permitir editar um serviço já publicado (apagar de verdade ficou fora: "pausar" já resolve, editar era o que faltava de verdade).

- **`professional_certifications` deixa de ser um gate**: `createService` não checa mais habilitação aprovada antes de publicar (migration `0085`, checagem removida em `lib/actions/services.ts`). Reabre `SELECT` pra qualquer um (era só dono+admin) e o bucket `professional-certifications` vira público — documento de habilitação passa a ser informação de confiança pro Tutor consultar, mesmo espírito de `professional_skills`/`professional_media`.
- **`notify_new_certification()`** (migration `0085`, mesmo padrão de `notify_new_incident()`): documento novo notifica todo Admin/Supervisor ativo — aparece em "Notificações" com link "Revisar habilitação" pra `/admin/habilitacoes`, que continua existindo, só que sem bloquear nada (aprovar/rejeitar só troca o selo que o Tutor vê).
- **`lib/domain/certification-status.ts`** (novo): calcula o status "efetivo" por categoria — **não é só o envio mais recente**. Uma vez aprovado, continua "✅ verificado" mesmo que o profissional reenvie um documento novo (CRMV vencido, por exemplo) — só troca quando o Admin decide sobre esse reenvio. Decisão explícita do usuário: "mantém validado até novo aprovar/rejeitar", pra não abrir uma lacuna de confiança durante a revisão. Testado ao vivo: aprovar → reenviar → selo continua "verificado" e o link aponta pro documento aprovado, não pro novo pendente; rejeitar o reenvio → aí sim vira "documentação não verificada".
- **`components/services/service-category-certification.tsx`** (novo): embutido no `ServiceForm`, só aparece quando a categoria selecionada exige habilitação (hoje só veterinário domiciliar) — mostra o status e permite anexar/reenviar documento sem sair da tela de Serviço.
- **Selo no Tutor**: `app/(tutor)/profissional/[profissionalId]/page.tsx` mostra o status por serviço regulamentado (inclusive "nenhum documento enviado" — transparência total, já que não tem mais aprovador bloqueando) e mantém o selo geral "Documentação verificada" no topo do perfil quando alguma categoria está aprovada.
- **Editar Serviço** (`updateService` em `lib/actions/services.ts`, `updateServiceSchema`): `components/services/services-manager.tsx` (novo) junta lista+formulário num client component só, com o estado de "qual serviço tá sendo editado". Clicar no lápis na lista abre o formulário pré-preenchido (inclusive campos específicos da categoria e adicionais); "Cancelar" ou salvar volta pro modo de criar novo. Adicionais são recriados do zero a cada edição (apaga e recria, mais simples que diff item a item).
**Verificação:** testado ao vivo de ponta a ponta — "Habilitações" sumiu do Perfil; selecionar "Veterinário domiciliar" no Serviço mostra os campos específicos + bloco de documentação; publicar sem documento nenhum funciona (sem bloqueio); envio de documento dispara notificação pra 5 contas admin/supervisor ativas (conferido direto no banco); aprovar/reenviar/rejeitar seguem a regra "mantém validado" corretamente, inclusive no card do serviço que o Tutor vê; editar o serviço "Pet sitter" (preço 80→95, salvo e confirmado no banco, revertido depois) manteve os campos específicos da categoria intactos. `tsc`/`eslint`/`next build` limpos. Achado à parte, não bug de código: durante a verificação a aba do navegador ficou presa num estado de Suspense travado depois de várias trocas de servidor/reload em sequência — confirmado que era só a aba (uma aba nova carregou tudo normalmente), não o app.
