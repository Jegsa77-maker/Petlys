-- Fix: "combined" tinha lat/lng/zip sem qualificar o alias da CTE de
-- origem — RETURNS TABLE cria variáveis PL/pgSQL de mesmo nome (lat, lng,
-- uf...) visíveis em toda a função, então a referência bare virava
-- ambígua ("column reference lat is ambiguous"), pego ao testar no
-- browser como Admin.
